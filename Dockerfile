...
WORKDIR /src
COPY gui/go_backend/go.mod gui/go_backend/go.sum ./
RUN go mod download
COPY gui/go_backend/main.go ./
RUN go build -o app
...
RUN xargs -a /tmp/vuln-library-purge apt-get purge -y \
